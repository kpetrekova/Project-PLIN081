...
def save_dataset_to_csv(df):
    ...
    if os.path.isfile("data/sh_dataset.csv"):
        df.to_csv("data/sh_dataset.csv", mode="a", sep="#", encoding="utf-8", header=False, index=False)
    else:
        df.to_csv("data/sh_dataset.csv", mode="w", sep="#", encoding="utf-8", header=True, index=False)
